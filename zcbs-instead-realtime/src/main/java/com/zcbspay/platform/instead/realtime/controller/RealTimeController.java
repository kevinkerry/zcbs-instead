...
public class RealTimeController {
	...
	public MessageBean payAndCollectApi(MessageBean messageBean) {
		...
		try {
			// 验签,解密
			HttpRequestParam httpRequestParam = new HttpRequestParam("data",
					JSONObject.fromObject(messageBean).toString());
		...
		try {
			...
			HttpRequestParam httpRequestParam2 = new HttpRequestParam("additBean",
					JSONObject.fromObject(prepareAdditbean(((AdditBean) JSONObject
							.toBean(JSONObject.fromObject(messageBean.getAddit()), AdditBean.class)).getMerId()))
							.toString());
	...
	private MessageBean encrypt(ResponseBaseBean responseBaseBean, MessageBean messageBean) {
		...
		HttpUtils httpUtils = new HttpUtils();
		try {
			HttpRequestParam httpRequestParam1 = new HttpRequestParam("enData",
					JSONObject.fromObject(responseBaseBean).toString());
			HttpRequestParam httpRequestParam2 = new HttpRequestParam("additBean",
					JSONObject.fromObject(prepareAdditbean(((AdditBean) JSONObject
							.toBean(JSONObject.fromObject(messageBean.getAddit()), AdditBean.class)).getMerId()))
							.toString());
	...
	/*
	 * @RequestMapping("i") public String index() { return "index"; }
	 * 
	 * @ResponseBody
	 * 
	 * @RequestMapping("paysubmit") public MessageBean
	 * paysubmit(RealTimePayReqBean realTimePayReqBean, EncryptData encryptData)
	 * { AdditBean additBean = new AdditBean();
	 * additBean.setMerId(realTimePayReqBean.getMerId());
	 * additBean.setAccessType(accessType); realTimePayReqBean.setEncryptData(
	 * encryptAndDecryptService.localEncrypt(additBean,
	 * JSONObject.fromObject(encryptData).toString())); return
	 * encryptData(realTimePayReqBean,realTimePayReqBean.getMerId());
	 * 
	 * }
	 * 
	 * @ResponseBody
	 * 
	 * @RequestMapping("collectsubmit") public MessageBean
	 * collectsubmit(RealTimeCollectReqBean realTimeCollectReqBean, EncryptData
	 * encryptData) {
	 * 
	 * AdditBean additBean = new AdditBean();
	 ...
	 * encryptAndDecryptService.localEncrypt(additBean,
	 * JSONObject.fromObject(encryptData).toString())); return
	 * encryptData(realTimeCollectReqBean,realTimeCollectReqBean.getMerId()); }
	 * 
	 * @ResponseBody
	 * 
	 * @RequestMapping("tradequery") public MessageBean
	 * tradequery(RealTimeQueryReqBean realTimeQueryReqBean) { // 确定处理类
	 * collectAndPaySerivce = (CollectAndPayService) SpringContextHelper
	 * .getBean(RealTimeTxnTypeEnum.getTxnTypeEnum(realTimeQueryReqBean.
	 * getTxnType()).getClassName()); MessageBean messageBean =new
	 * MessageBean();
	 * messageBean.setData(JSONObject.fromObject(realTimeQueryReqBean).toString(
	 * )); collectAndPaySerivce.invoke(messageBean); return
	 * encryptData(realTimeQueryReqBean,null); }
	 * 
	 * private MessageBean encryptData(Object object,String merid) { MessageBean
	 * requestBean=null; AdditBean additBean = new AdditBean();
	 * additBean.setAccessType(accessType);
	 * additBean.setMerId(merid==null?MER_ID:merid);
	 * additBean.setEncryMethod(encryMethod); HttpUtils httpUtils = new
	 * HttpUtils(); try { Map<String, Object> riskInfo = new TreeMap<String,
	 * Object>(); riskInfo.put("random", RiskInfoUtils.randomInt(32));
	 * riskInfo.put("timestamp", DateUtils.geCurrentDateTimeStr());
	 * riskInfo.put("deviceID", RiskInfoUtils.getMacAddress());
	 * additBean.setRiskInfo(JSONObject.fromObject(riskInfo).toString());
	 * 
	 * HttpRequestParam httpRequestParam1= new
	 * HttpRequestParam("enData",JSONObject.fromObject(object).toString());
	 * HttpRequestParam httpRequestParam2= new
	 * HttpRequestParam("additBean",JSONObject.fromObject(additBean).toString())
	 * ; List<HttpRequestParam> listen = new ArrayList<>();
	 * listen.add(httpRequestParam1); listen.add(httpRequestParam2); String url
	 * =urlBean.getEncryptUrl();
	 * 
	 * httpUtils.openConnection(); String responseContent =
	 * httpUtils.executeHttpPost(url,listen,Constants.Encoding.UTF_8);
	 * 
	 * requestBean=(MessageBean)
	 * JSONObject.toBean(JSONObject.fromObject(responseContent),MessageBean.
	 * class); return requestBean; } catch (Exception e) { e.printStackTrace();
	 * return null; }finally { httpUtils.closeConnection(); } }
	 */
